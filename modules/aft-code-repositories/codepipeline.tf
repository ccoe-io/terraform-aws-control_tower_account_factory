...
resource "aws_codepipeline" "codestar_account_request" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ConnectionArn        = lookup({ github = local.connection_arn.github, bitbucket = local.connection_arn.bitbucket, gitlab = local.connection_arn.gitlab,githubenterprise = local.connection_arn.githubenterprise }, var.vcs_provider)
